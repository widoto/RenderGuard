...
|--|----------------------|-------------|----------------|
| **Approach** | Declared-color threshold | Rule-based + partial OCR | Render-diff (CR + ΔE₀₀) |
...
| **Resources** | CPU | CPU (+ Tesseract OCR) | CPU (no OCR, no ML) |

- No OCR, no ML model. Pure pixel measurement on CPU.
...
(2025-04-17) — 16 months, 2 total commits, never corrected. The detector
...
Code: AGPL-3.0 (due to PyMuPDF dependency)

Test corpus (`corpus/`): AGPL-3.0 — synthetic single-page PDFs generated by
`generate_corpus.py`.
